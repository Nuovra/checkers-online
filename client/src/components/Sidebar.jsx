...
function LogoSVG({ size = 44 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" style={{ flexShrink: 0, filter: 'drop-shadow(0 4px 12px rgba(0,0,0,0.5))' }}>
      <defs>
        ...
        <radialGradient id="sb2" cx="38%" cy="30%" r="65%"><stop offset="0%" stopColor="#dd2222"/><stop offset="100%" stopColor="#6a0000"/></radialGradient>
      </defs>
      <ellipse cx="50" cy="58" rx="43" ry="9" fill="rgba(0,0,0,0.35)"/>
...
export default function Sidebar() {
  ...
  function go(item) { if (isGuest && !item.guest) { navigate('/login?signup=1'); return; } navigate(item.path); }
    ...
    <>
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          {NAV.map(item => (
            <button key={item.path} className={`sidebar-item${active(item.path) ? ' active' : ''}${isGuest && !item.guest ? ' locked' : ''}`} onClick={() => go(item)}>
        ...
        <div className="sidebar-bottom">
          <div className="sidebar-online"><span className="online-dot" /><span>{onlineCount.toLocaleString()} online</span></div>
          <div className="sidebar-user" onClick={() => isGuest ? navigate('/login?signup=1') : navigate(`/profile/${user.username}`)}>
            <div className="sidebar-avatar">{isGuest ? <span style={{ fontWeight: 900, color: '#000' }}>G</span> : avatar ? <img src={avatar} alt="" /> : user.username?.[0]?.toUpperCase()}</div>
            <div className="sidebar-user-info">
              <div className="sidebar-username">{isGuest ? 'Guest' : user.username}{!isGuest && user.country && <FlagImg code={user.country} size={12} />}</div>
              <div className="sidebar-elo">{isGuest ? <span style={{ color: 'var(--accent)' }}>Sign up →</span> : `${user.elo} ELO`}</div>
            </div>
          </div>
          <button className="sidebar-logout" onClick={() => { logout(); navigate('/login'); }}>{isGuest ? '↩ Log In' : '↩ Log out'}</button>
        </div>
      </aside>
      <nav className="mobile-nav">
        {NAV.map(item => (
          <button key={item.path} className={`mobile-nav-item${active(item.path) ? ' active' : ''}`} onClick={() => go(item)}>
            <span className="mobile-nav-icon">{item.icon}</span><span className="mobile-nav-label">{item.label}</span>
        ...
        <button className="mobile-nav-item" onClick={() => isGuest ? navigate('/login?signup=1') : navigate(`/profile/${user.username}`)}>
          <span className="mobile-nav-icon">{isGuest ? '👤' : avatar ? <img src={avatar} alt="" style={{ width: 22, height: 22, borderRadius: '50%', objectFit: 'cover' }} /> : '👤'}</span>
